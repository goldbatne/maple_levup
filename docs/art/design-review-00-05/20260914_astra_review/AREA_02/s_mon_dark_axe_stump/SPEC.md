# GENERATION SPEC — 다크 엑스텀프 / 어둠의 뿌리
...
- 몬스터 이름: 다크 엑스텀프
- monster image RUID: `fd35894dd92c4e3b8885e59b0f4d3394`
- skill_id: `s_mon_dark_axe_stump`
- 최종 스킬 이름: **어둠의 뿌리**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: INT 계수 1.5로 반경 4 범위 대상 제한 없음 피해; 5초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.12초; 지속 0.82/0.82초

## 원작/지역 연결

[P][A] 다크 엑스텀프와 뿌리/어둠은 이름상 연결되나 고유 능력 근거는 확인되지 않았다.

## 시각 번역

- 핵심 소재: 지면을 찢고 솟는 검은 뿌리 갈퀴와 보랏빛 흙먼지. ‘다크 엑스텀프 / 어둠의 뿌리’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 암목 흑갈 #3A3038; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 저주 보라 #80579A; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 또는 지정 충돌점의 지면 중심에서 ‘어둠의 뿌리’의 지면을 찢고 솟는 검은 뿌리 갈퀴와 보랏빛 흙먼지이 먼저 식별되도록 발생
- 진행 방향: 지면을 찢고 솟는 검은 뿌리 갈퀴와 보랏빛 흙먼지이 중심에서 방사 또는 수직으로 확장하고 절정 뒤 파편은 원점으로 되감기지 않음
- 대상 표현: 확정 효과 ‘INT 계수 1.5로 반경 4 범위 대상 제한 없음 피해; 5초 재사용; 5장 보유 시 계수 ×1.8’의 범위 안 대상만 표시하며 중심 타격광과 바깥 범위 경계를 분리해 다중 피격을 읽히게 함
