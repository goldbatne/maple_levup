...
- 몬스터 이름: 파이어보어
- monster image RUID: `67523b05f66f470cb86b2c6cc46af861`
- skill_id: `s_mon_fire_boar`
- 최종 스킬 이름: **화염 돌풍**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: ATK 계수 1.7로 반경 4.5 범위 대상 제한 없음 피해; 6초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.1초; 지속 0.52/0.7초

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 파이어보어의 모델 ID·RUID와 페리온 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 낮게 휘는 불꽃 혀와 돌진 방향으로 흩는 불티. ‘파이어보어 / 화염 돌풍’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 화염 주황 #F06A2B; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 고열 황색 #FFD15C; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 몸통 중심과 발밑 사이의 고정 피봇에서 ‘화염 돌풍’의 낮게 휘는 불꽃 혀와 돌진 방향으로 흩는 불티이 먼저 나타남
- 진행 방향: 낮게 휘는 불꽃 혀와 돌진 방향으로 흩는 불티의 고유 결을 따라 한 번 확장·수축하며 불필요한 화면 횡단은 하지 않음
- 대상 표현: 확정 효과 ‘ATK 계수 1.7로 반경 4.5 범위 대상 제한 없음 피해; 6초 재사용; 5장 보유 시 계수 ×1.8’가 지정한 대상만 표현하고 몬스터·플레이어 본체 실루엣은 넣지 않음
