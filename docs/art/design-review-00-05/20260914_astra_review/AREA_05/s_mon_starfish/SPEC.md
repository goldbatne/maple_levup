...
- 몬스터 이름: 불가사리
- monster image RUID: `03351051e9e54fca87469eaa47c5398b`
- skill_id: `s_mon_starfish`
- 최종 스킬 이름: **별가시 회전**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: ATK 계수 1.95로 반경 4 범위 최대 3대상 피해; 6초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.06초; 지속 0.55/0.68초

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 불가사리의 모델 ID·RUID와 노틸러스 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 별 모양 가시가 회전하며 남기는 다섯 갈래 수중 잔상. ‘불가사리 / 별가시 회전’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 불가사리 주황 #F39B45; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 바닷물 청록 #5CC8C4; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 또는 지정 충돌점의 지면 중심에서 ‘별가시 회전’의 별 모양 가시가 회전하며 남기는 다섯 갈래 수중 잔상이 먼저 식별되도록 발생
- 진행 방향: 별 모양 가시가 회전하며 남기는 다섯 갈래 수중 잔상이 중심에서 방사 또는 수직으로 확장하고 절정 뒤 파편은 원점으로 되감기지 않음
- 대상 표현: 확정 효과 ‘ATK 계수 1.95로 반경 4 범위 최대 3대상 피해; 6초 재사용; 5장 보유 시 계수 ×1.8’의 범위 안 대상만 표시하며 중심 타격광과 바깥 범위 경계를 분리해 다중 피격을 읽히게 함
