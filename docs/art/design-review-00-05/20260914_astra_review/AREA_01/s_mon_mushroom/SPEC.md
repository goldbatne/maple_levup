...
- 몬스터 이름: 주황 버섯
- monster image RUID: `a95cfed2c8fe4d2cb64cbb62db051f92`
- skill_id: `s_mon_mushroom`
- 최종 스킬 이름: **포자 살포**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: INT 계수 1.1로 반경 2.5 범위 대상 제한 없음 피해; 4초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 3개; 타입 animationclip/animationclip/animationclip; 스타일 clip/clip/clip; 지연 0/0.08/0.16초; 지속 0.55/0.6/0.65초

## 원작/지역 연결

[P][A] 버섯과 포자는 일반적으로 연결되나 주황버섯의 고유 능력으로 확인되지는 않았다.

## 시각 번역

- 핵심 소재: 작은 포자 구름 세 덩이와 부드럽게 흩어지는 점 입자. ‘주황 버섯 / 포자 살포’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 버섯 베이지 #D8B679; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 새싹 연두 #9BCB58; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 또는 지정 충돌점의 지면 중심에서 ‘포자 살포’의 작은 포자 구름 세 덩이와 부드럽게 흩어지는 점 입자이 먼저 식별되도록 발생
- 진행 방향: 작은 포자 구름 세 덩이와 부드럽게 흩어지는 점 입자이 중심에서 방사 또는 수직으로 확장하고 절정 뒤 파편은 원점으로 되감기지 않음
- 대상 표현: 확정 효과 ‘INT 계수 1.1로 반경 2.5 범위 대상 제한 없음 피해; 4초 재사용; 5장 보유 시 계수 ×1.8’의 범위 안 대상만 표시하며 중심 타격광과 바깥 범위 경계를 분리해 다중 피격을 읽히게 함
