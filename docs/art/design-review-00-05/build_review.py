...
OUT=ROOT/'design-review-00-05'/'20260914_astra_review'
...
for a in range(6):
    tag=f'AREA_{a:02}'; run='20260913_211019' if a==0 else '20260913_223849'
    rd=ROOT/'chatgpt-work-runs'/tag/run; ip=rd/'_input'/f'{tag}_IMAGES_INPUT'; zp=rd/f'{tag}_IMAGES_OUTPUT.zip'
    area=[]
    with zipfile.ZipFile(zp) as z:
        ...
        for md in sorted((ip/'monsters').iterdir()):
            ...
            icons=sorted(n for n in names if '/ICON/' in n and n.endswith('.png') and ('/'+md.name+'/') in n)
            if not fs and not icons:
                fs=sorted(n for n in names if '/VFX/' in n and n.endswith('.png') and f'_{sid}_' in n)
                icons=sorted(n for n in names if '/ICON/' in n and n.endswith('.png') and f'_{sid}_' in n)
            rec=dict(area=tag,monster=mid,monster_name=mname,skill=sid,skill_name=sname,ruid=ruid,zip=str(zp),zip_sha=sha(zp.read_bytes()),frames=[],icons=[],references=re.findall(r'### (.*?) / `(skill/[^`]+)`',brief),material='\n'.join(l for l in spec.splitlines() if any(t in l for t in ['핵심 소재:','진행 방향:','아이콘 핵심','- VFX:'])))
            ...
            row=Image.new('RGB',(2030,190),(25,28,35));dr=ImageDraw.Draw(row);dr.text((5,2),f'{tag} {mname} / {sname}',font=FONT,fill='white');row.paste(tile(monster,140),(0,36))
        ...
        sheet=Image.new('RGB',(2030,len(area)*190));
