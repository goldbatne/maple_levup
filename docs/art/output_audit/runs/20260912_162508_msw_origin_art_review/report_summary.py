...
def main():
    f=ImageFont.truetype('C:/Windows/Fonts/malgun.ttf',24)
    for mid,a,notes in [
      ('m_king_bloctopus','area_11',['검수 결론: PROJECTILE 4장만 SPEC_MISMATCH. CAST / ICON 원화 유지.','CAST의 눈 없는 왕관 블록과 PROJECTILE의 눈·포구·분홍 몸통을 구분한다.']),
      ('m_toy_trojan','area_12',['검수 결론: CAST / ICON 모두 NO_OBVIOUS_ISSUE. 기존 아트 보류 판단 정정.','CAST의 원형은 태엽 구멍·바퀴 축이다. 확대본에서 눈 달린 머리/상체를 확인하지 못했다.'])]:
        im=Image.open(RUN/'evidence'/f'{a}_{mid}_comparison.png');c=Image.new('RGB',(1800,im.height+130),(24,29,36));c.paste(im,(0,130));d=ImageDraw.Draw(c)
        for j,n in enumerate(notes):d.text((20,15+j*45),n,font=f,fill='#ffe59c')
        c.save(RUN/'evidence'/f'{a}_{mid}_ruling.png')
    # Preserve exact MCP read-only response as a session observation, without inventing its timestamp.
    dump('MCP_READ_ONLY_OBSERVATION.json',{'tool':'mcp__msw_maker_mcp__maker_get_world_info','result':{'status':'ok','isGroup':False,'worldId':'6ab29f6a728e4b33931679be708ca6b1','userId':'20372000905997288'},'provenance':'Actual tool response observed in this audit conversation; exact call timestamp not recorded','scope':'Connection identity only; not a remote entity/model enumeration, refresh, play or runtime validation','resource_method':'msw-search validated public read-only getResourcesBatch wrapper, followed by exact returned official thumbnail GIF URL download'})
    prevtech=[r for r in readcsv(OLD/'ACTION_ITEMS_UPDATED.csv') if r['area_id'] in ['area_11','area_12'] and r['source']=='PREVIOUS_MECHANICAL_AUDIT_REUSED']
    for r in prevtech:r['reuse_note']='같은 원본 ZIP 해시의 기존 기술 판정. missing은 기대 canonical 경로 기준이며, 다른 내부 경로에 있는 실제 납품 원화가 사라졌다는 뜻이 아님.'
    writecsv('HELD_TECHNICAL_ITEMS_REUSED.csv',prevtech)
    # All source links and observations are now fixed; summarize the review, not a promise of runtime success.
    summary=f'''# MSW 원본 ↔ 몬스터 스킬 아트 적합성 검수

실행 폴더: `{RUN.as_posix()}`  
보고서 작성: {datetime.now().isoformat(timespec='seconds')} (로컬 KST)
...
- Area별 INPUT 버전·경로·해시, OUTPUT 경로·해시, 선택 근거는 [AREA_SELECTION.csv]({(RUN/'AREA_SELECTION.csv').as_posix()})에 있다. 원본 OUTPUT 후보 해시 전부는 ORIGINAL_OUTPUT_HASHES.csv에 기록했다.
...
근거: [SOURCE_CHECK.csv]({(RUN/'SOURCE_CHECK.csv').as_posix()}), resource_batch_*.json, resource_metadata.json, download_log.json, model_snapshots.json, source_copies/*.gif, boards/area_*_source_and_all_frames.png, boards/source_detail_*.png.
...
근거: [ART_REVIEW.csv]({(RUN/'ART_REVIEW.csv').as_posix()}) (209행), CONTRACT_SNAPSHOTS.json, PROJECT_DATA_CHECK.csv. 기존 관찰 기록은 보조로 재사용하되 실제 전체 프레임을 다시 열었고, 오류가 드러난 King/Toy 서술은 새 관찰로 교체했다.
...
비교: [AREA11 판정 이미지]({(RUN/'evidence/area_11_m_king_bloctopus_ruling.png').as_posix()}).
...
비교: [AREA12 판정 이미지]({(RUN/'evidence/area_12_m_toy_trojan_ruling.png').as_posix()}), [F00·F02·F03 원본 크기 상세]({(RUN/'evidence/toy_cast_00_02_03_detail.png').as_posix()}), [F01 원본]({(RUN/'evidence/toy_cast_F01_original.png').as_posix()}).
...
근거: STYLE_REFERENCE_CHECK.csv, PREVIEW_CHECK.csv, boards/style_reference_candidates.png, boards/area00_approved_actual.png, boards/preview_review_01..05.png, [AREA11 Preview 문구 겹침]({(RUN/'evidence/area_11_overview_top_detail.png').as_posix()}).
...
조치 목록: [ACTION_ITEMS.csv]({(RUN/'ACTION_ITEMS.csv').as_posix()}). 수정·기술 보정·기획 질문·선택적 예외 승인을 구분했고 이번에는 어느 조치도 실행하지 않았다. 생성 이력 부재만으로 재생성을 요구한 항목은 없다.
...
'''
    (RUN/'REVIEW_SUMMARY.md').write_text(summary,encoding='utf8')
    print('SUMMARY_WRITTEN',len(summary),'chars')
