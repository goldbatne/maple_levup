...
sources=['exec-bee4f093-eae4-4157-889a-65282b1064a1.png','exec-d5ac473b-85d3-45a7-9e96-0a9a69956829.png'];base=Path('C:/Users/dddd/.codex/generated_images/01a0955f-0d5b-7931-a751-c770b2046bd8')
for n,src in enumerate(sources,1):shutil.copy2(base/src,R/'evidence'/f'MUSHROOM_CAST_FINAL_TRIAL_SOURCE{n}.png')
...
alpha=np.clip((lum-2)/253,0,1);rgb=np.clip((a-2)/np.maximum(alpha[:,:,None],1/255),0,255);rgba=Image.fromarray(np.dstack([rgb,alpha*255]).astype('uint8'),'RGBA')
...
preview.save(R/'evidence/MUSHROOM_CAST_ATTEMPT3_3_BACKGROUNDS.png');(R/'evidence/MUSHROOM_CAST_ATTEMPT3_TECHNICAL.json').write_text(json.dumps({'method':'black matte alpha=clip((maxRGB-2)/253), RGB unpremultiply; no illustration changes, no autocrop/recenter. Not universal dark-material method.','files':rows},indent=2),encoding='utf-8');print(json.dumps(rows))
