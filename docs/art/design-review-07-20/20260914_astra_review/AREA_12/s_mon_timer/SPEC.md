...
- monster_id: `m_timer`
- 몬스터 이름: 타이머
- monster image RUID: `cbdd230e5cd34b7b9eeb1cd206d19f34`
- skill_id: `s_mon_timer`
- 최종 스킬 이름: **시간 정지 충격**
- 스킬 타입: **액티브**
- 보스 여부: 보스
- 실제 현재 효과: INT 계수 4로 반경 6.2 범위 대상 제한 없음 피해; 10초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.2초; 지속 0.78/0.88초

## 원작/지역 연결

[P][A] 타이머와 시간 테마는 직접 연결된다.

## 시각 번역

- 핵심 소재: 정지한 시곗바늘에서 각진 시간 충격륜이 터지는 순간. ‘타이머 / 시간 정지 충격’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 시계 황동 #C49B52; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 정지 청백 #C5EAF3; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 또는 지정 충돌점의 지면 중심에서 ‘시간 정지 충격’의 정지한 시곗바늘에서 각진 시간 충격륜이 터지는 순간이 먼저 식별되도록 발생
- 진행 방향: 정지한 시곗바늘에서 각진 시간 충격륜이 터지는 순간이 중심에서 방사 또는 수직으로 확장하고 절정 뒤 파편은 원점으로 되감기지 않음
- 대상 표현: 확정 효과 ‘INT 계수 4로 반경 6.2 범위 대상 제한 없음 피해; 10초 재사용; 5장 보유 시 계수 ×1.8’의 범위 안 대상만 표시하며 중심 타격광과 바깥 범위 경계를 분리해 다중 피격을 읽히게 함
