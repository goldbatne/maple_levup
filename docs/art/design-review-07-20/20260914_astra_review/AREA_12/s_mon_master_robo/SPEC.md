# GENERATION SPEC — 마스터 로보 / 정밀 태엽회로
...
- 몬스터 이름: 마스터 로보
- monster image RUID: `f5d65e28cb0e4990bf5727015f22e0f1`
- skill_id: `s_mon_master_robo`
- 최종 스킬 이름: **정밀 태엽회로**
- 스킬 타입: **패시브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: 보유만 해도 DEX +1; 중복 1장당 +0.25, 5장 최대 +2
- 실제 현재 동작: 현행 런타임 VFX 없음; 인벤토리/스킬 UI 아이콘으로만 표시

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 마스터 로보의 모델 ID·RUID와 루디브리엄 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 정밀 톱니 세 개와 회로선이 동심으로 맞물리는 코어. ‘마스터 로보 / 정밀 태엽회로’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 기계 은회 #87939E; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 회로 청록 #54D0C1; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: ‘정밀 태엽회로’의 정밀 톱니 세 개와 회로선이 동심으로 맞물리는 코어을 시전자 몸 둘레의 닫힌 윤곽에 고정하며 실제 주 산출물은 아이콘 중심
- 진행 방향: 정밀 톱니 세 개와 회로선이 동심으로 맞물리는 코어의 대표 결만 바깥으로 8~15% 맥동하고 이동 궤적은 만들지 않음
- 대상 표현: 확정 효과 ‘보유만 해도 DEX +1; 중복 1장당 +0.25, 5장 최대 +2’의 시전자 자신만 표시하며 현행 런타임에 없는 공격·투사체·새 버프 연출을 암시하지 않음
