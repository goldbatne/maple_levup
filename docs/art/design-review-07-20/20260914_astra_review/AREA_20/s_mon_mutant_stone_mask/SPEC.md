# GENERATION SPEC — 변형된 스톤마스크 / 발굴지의 석면
...
- 몬스터 이름: 변형된 스톤마스크
- monster image RUID: `f020a220a2ad40a28d4920b790b32f3c`
- skill_id: `s_mon_mutant_stone_mask`
- 최종 스킬 이름: **발굴지의 석면**
- 스킬 타입: **패시브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: 보유만 해도 DEF +1; 중복 1장당 +0.25, 5장 최대 +2
- 실제 현재 동작: 현행 런타임 VFX 없음; 인벤토리/스킬 UI 아이콘으로만 표시

## 원작/지역 연결

[P][A] 변형된 스톤마스크와 황혼의 페리온 배치는 확인된다. 그러나 ‘석면’은 한국어에서 일반적으로 asbestos(石綿)를 뜻한다.

## 시각 번역

- 핵심 소재: 깨진 석면 조각과 발굴지 문양이 겹치는 암석 방호. ‘변형된 스톤마스크 / 발굴지의 석면’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 석면 회갈 #777067; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 발굴 황토 #B9945B; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: ‘발굴지의 석면’의 깨진 석면 조각과 발굴지 문양이 겹치는 암석 방호을 시전자 몸 둘레의 닫힌 윤곽에 고정하며 실제 주 산출물은 아이콘 중심
- 진행 방향: 깨진 석면 조각과 발굴지 문양이 겹치는 암석 방호의 대표 결만 바깥으로 8~15% 맥동하고 이동 궤적은 만들지 않음
- 대상 표현: 확정 효과 ‘보유만 해도 DEF +1; 중복 1장당 +0.25, 5장 최대 +2’의 시전자 자신만 표시하며 현행 런타임에 없는 공격·투사체·새 버프 연출을 암시하지 않음
...
- ‘석면’은 현재 확정 문자열을 유지하되 asbestos 섬유를 그리지 말고 스톤마스크/발굴지의 돌 표면 모티브만 사용한다.
