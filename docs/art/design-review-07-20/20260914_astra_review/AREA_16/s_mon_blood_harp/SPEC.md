...
- 몬스터 이름: 블러드 하프
- monster image RUID: `8c2ec5bf58894061a19479ed7545637b`
- skill_id: `s_mon_blood_harp`
- 최종 스킬 이름: **핏빛 깃울림**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: INT 계수 4.75로 반경 5.6 범위 최대 4대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.18초; 지속 0.68/0.78초

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 블러드 하프의 모델 ID·RUID와 미나르숲 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 핏빛 깃털이 음파 고리와 함께 떨리는 날카로운 울림. ‘블러드 하프 / 핏빛 깃울림’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 핏빛 적색 #A93F52; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 음파 자주 #76527C; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 몸통 중심과 발밑 사이의 고정 피봇에서 ‘핏빛 깃울림’의 핏빛 깃털이 음파 고리와 함께 떨리는 날카로운 울림이 먼저 나타남
- 진행 방향: 핏빛 깃털이 음파 고리와 함께 떨리는 날카로운 울림의 고유 결을 따라 한 번 확장·수축하며 불필요한 화면 횡단은 하지 않음
- 대상 표현: 확정 효과 ‘INT 계수 4.75로 반경 5.6 범위 최대 4대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8’가 지정한 대상만 표현하고 몬스터·플레이어 본체 실루엣은 넣지 않음
