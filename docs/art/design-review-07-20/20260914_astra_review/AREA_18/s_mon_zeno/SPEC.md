...
- monster_id: `m_zeno`
- 몬스터 이름: 제노
- monster image RUID: `689bc520bc2544abb62ec505ca91aab4`
- skill_id: `s_mon_zeno`
- 최종 스킬 이름: **제노의 중력장**
- 스킬 타입: **액티브**
- 보스 여부: 보스
- 실제 현재 효과: INT 계수 5.7로 반경 7.2 범위 대상 제한 없음 피해; 12초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/.24초; 지속 .82/.96초

## 원작/지역 연결

[P][A] 제노와 외계 지역은 연결되나 ‘중력장’ 능력은 확인되지 않았다.
...
- 효과 발생 위치: 시전자 몸통 중심과 발밑 사이의 고정 피봇에서 ‘제노의 중력장’의 외계 코어 주위 공간이 오목하게 접히는 다중 중력륜이 먼저 나타남
- 진행 방향: 외계 코어 주위 공간이 오목하게 접히는 다중 중력륜의 고유 결을 따라 한 번 확장·수축하며 불필요한 화면 횡단은 하지 않음
- 대상 표현: 확정 효과 ‘INT 계수 5.7로 반경 7.2 범위 대상 제한 없음 피해; 12초 재사용; 5장 보유 시 계수 ×1.8’가 지정한 대상만 표현하고 몬스터·플레이어 본체 실루엣은 넣지 않음
