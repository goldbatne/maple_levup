...
- monster_id: `m_mateon`
- 몬스터 이름: 마티안
- monster image RUID: `53043af9433844258a090f3b231ce39f`
- skill_id: `s_mon_mateon`
- 최종 스킬 이름: **마티안 광선탄**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: INT 계수 5.3로 단일 대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/.2초; 지속 .65/.78초; 투사체 22db356cc60940089b7f5d592c7d5813가 목표 방향으로 이동

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 마티안의 모델 ID·RUID와 지구방위본부 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 외계 수정핵에서 발사되는 둥근 황록 광선탄. ‘마티안 / 마티안 광선탄’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 외계 황록 #C8DD59; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 우주 청록 #55B9B0; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 전방 발사 피봇에서 ‘마티안 광선탄’의 외계 수정핵에서 발사되는 둥근 황록 광선탄이 완성되며 몬스터 본체 자리는 비워 둠
- 진행 방향: 외계 수정핵에서 발사되는 둥근 황록 광선탄이 오른쪽 한 방향으로 이동하고 꼬리·잔상은 발사점 쪽에만 남김
- 대상 표현: 확정 효과 ‘INT 계수 5.3로 단일 대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8’의 전방 피격 대상과 충돌점을 표시하며 발사체 실루엣과 충돌광을 서로 다른 프레임으로 분리
