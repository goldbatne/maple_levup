# GENERATION SPEC — 화이트팽 / 화이트팽의 빙설 송곳니
...
- 몬스터 이름: 화이트팽
- monster image RUID: `aaa3b506804d4418b025da47dbb65b3f`
- skill_id: `s_mon_white_fang`
- 최종 스킬 이름: **화이트팽의 빙설 송곳니**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: ATK 계수 2.9로 반경 4.8 범위 최대 3대상 피해; 7초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.15초; 지속 0.65/0.72초

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 화이트팽의 모델 ID·RUID와 엘나스 산맥 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.
...
- 효과 발생 위치: 시전자 전방 15% 지점에서 ‘화이트팽의 빙설 송곳니’의 송곳니 두 개가 얼음 능선처럼 교차하는 빙설 타격이 시작되어 발밑 높이와 공격축을 맞춤
- 진행 방향: 송곳니 두 개가 얼음 능선처럼 교차하는 빙설 타격이 오른쪽으로 압축·가속하고 충돌점에서 짧게 넓어진 뒤 반대 방향 파편 없이 소멸
- 대상 표현: 확정 효과 ‘ATK 계수 2.9로 반경 4.8 범위 최대 3대상 피해; 7초 재사용; 5장 보유 시 계수 ×1.8’의 전방 피격 대상과 최초 충돌점을 표시하고 타격광은 대상 중심을 가리지 않는 외곽 고리로 제한
