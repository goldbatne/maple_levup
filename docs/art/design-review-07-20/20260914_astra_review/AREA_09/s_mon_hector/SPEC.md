...
- monster_id: `m_hector`
- 몬스터 이름: 헥터
- monster image RUID: `7dcc64a4725b4777b8f86a5404b4d4cc`
- skill_id: `s_mon_hector`
- 최종 스킬 이름: **설원 늑대 발톱**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: ATK 계수 2.75로 단일 대상 피해; 6초 재사용; 5장 보유 시 계수 ×1.8; 6.5 거리 돌진
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.12초; 지속 0.58/0.62초; 시전자가 목표 방향으로 돌진

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 헥터의 모델 ID·RUID와 엘나스 산맥 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 늑대 발톱 세 갈래가 눈가루를 베어내는 전방 참격. ‘헥터 / 설원 늑대 발톱’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 설원 회청 #798D9F; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 눈가루 백색 #ECFAFF; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 전방 15% 지점에서 ‘설원 늑대 발톱’의 늑대 발톱 세 갈래가 눈가루를 베어내는 전방 참격이 시작되어 발밑 높이와 공격축을 맞춤
- 진행 방향: 늑대 발톱 세 갈래가 눈가루를 베어내는 전방 참격이 오른쪽으로 압축·가속하고 충돌점에서 짧게 넓어진 뒤 반대 방향 파편 없이 소멸
- 대상 표현: 확정 효과 ‘ATK 계수 2.75로 단일 대상 피해; 6초 재사용; 5장 보유 시 계수 ×1.8; 6.5 거리 돌진’의 전방 피격 대상과 최초 충돌점을 표시하고 타격광은 대상 중심을 가리지 않는 외곽 고리로 제한
