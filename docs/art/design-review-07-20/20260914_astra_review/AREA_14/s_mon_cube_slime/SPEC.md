...
- 몬스터 이름: 큐브슬라임
- monster image RUID: `e93e27480f1043ef8e51fa3b0906a4d9`
- skill_id: `s_mon_cube_slime`
- 최종 스킬 이름: **연금 큐브막**
- 스킬 타입: **패시브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: 보유만 해도 INT +1; 중복 1장당 +0.25, 5장 최대 +2
- 실제 현재 동작: 현행 런타임 VFX 없음; 인벤토리/스킬 UI 아이콘으로만 표시

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 큐브슬라임의 모델 ID·RUID와 마가티아 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 투명 연금 큐브가 점액처럼 휘며 겹치는 사각 보호막. ‘큐브슬라임 / 연금 큐브막’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 연금 청록 #5AC6AF; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 큐브 백청 #CFF8EA; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: ‘연금 큐브막’의 투명 연금 큐브가 점액처럼 휘며 겹치는 사각 보호막을 시전자 몸 둘레의 닫힌 윤곽에 고정하며 실제 주 산출물은 아이콘 중심
- 진행 방향: 투명 연금 큐브가 점액처럼 휘며 겹치는 사각 보호막의 대표 결만 바깥으로 8~15% 맥동하고 이동 궤적은 만들지 않음
- 대상 표현: 확정 효과 ‘보유만 해도 INT +1; 중복 1장당 +0.25, 5장 최대 +2’의 시전자 자신만 표시하며 현행 런타임에 없는 공격·투사체·새 버프 연출을 암시하지 않음
