# GENERATION SPEC — 추억의 신관 / 추억의 기도파
...
- 몬스터 이름: 추억의 신관
- monster image RUID: `699e78db5d3a48578388d33dc333258a`
- skill_id: `s_mon_memory_monk_trainee`
- 최종 스킬 이름: **추억의 기도파**
- 스킬 타입: **액티브**
- 보스 여부: 일반 몬스터
- 실제 현재 효과: INT 계수 5.05로 단일 대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8
- 실제 현재 동작: layer_ruids 2개; 타입 animationclip/animationclip; 스타일 clip/clip; 지연 0/0.2초; 지속 .72/.82초

## 원작/지역 연결

[P][A] 현행 모델/방 배치에서 추억의 신관의 모델 ID·RUID와 시간의 신전 배치를 확인했다. 스킬명은 몬스터 고유명·종족명 또는 지역 핵심어를 직접 사용한다. 원작의 공식 스킬명으로 확인된 것은 아니다.

## 시각 번역

- 핵심 소재: 작은 기도 구슬이 모여 앞으로 번지는 파동. ‘추억의 신관 / 추억의 기도파’ 전용 소재이며 몬스터 본체는 VFX에 직접 넣지 않음
- 주 색상: 기도 청백 #AFCFD4; 핵심 실루엣의 중간톤과 어두운 외곽에 사용
- 보조 색상: 추억 금갈 #C3A46A; 타격 코어·잔상·소수 입자에만 사용해 같은 Area의 다른 몬스터와 구분
- 효과 발생 위치: 시전자 몸통 중심과 발밑 사이의 고정 피봇에서 ‘추억의 기도파’의 작은 기도 구슬이 모여 앞으로 번지는 파동이 먼저 나타남
- 진행 방향: 작은 기도 구슬이 모여 앞으로 번지는 파동의 고유 결을 따라 한 번 확장·수축하며 불필요한 화면 횡단은 하지 않음
- 대상 표현: 확정 효과 ‘INT 계수 5.05로 단일 대상 피해; 8초 재사용; 5장 보유 시 계수 ×1.8’가 지정한 대상만 표현하고 몬스터·플레이어 본체 실루엣은 넣지 않음
