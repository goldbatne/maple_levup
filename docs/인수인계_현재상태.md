...
- 갱신: 2026-08-29 (T40-3 커닝시티 철근 데코)
- 대상: 이 프로젝트를 **처음 받는 사람 또는 AI**

> **이 문서의 목적 하나.** 기획서는 "이렇게 만들 것"을 적은 문서고, 코드는 "이렇게 만들어진 것"이다.
> 둘은 지금 **6군데에서 다르다** — 문서·지시에 있으나 없는 것 5건(C, D, F, J, **L**) + 문서 근거 없이 있는 것 1건(H).
...
| **L** | `RoomTable`·`MonsterTable`의 **커닝시티 몬스터 5종**(옥토퍼스·스티지·주니어 레이스·셰이드·레이스) | 데이터·방·맵·데코는 전부 있는데 **`.model`이 없다.** 지금 area_04에 들어가면 `[LEA-3028] MissingModel`이 도배되고 몬스터가 한 마리도 안 선다 | T39-1에서 데이터만 먼저 넣었다. `Snail.model`을 복제해 RUID만 가는 작업이고(T36-2와 같은 방식), ⚠ **클립에 이름이 안 붙어 대조 시트로 골라야 한다**(T19-3 / T36-2). **`LandmarkTable`에 `30,area,area_04`를 일부러 안 넣어 둔 것**이 지금 이 방으로 못 들어가게 막고 있다 |
...
| # | 무엇이었나 | 어떻게 해소됐나 |
|---|---|---|
| B | 기획서 §4.5 랜드마크 **구역 개방** — 판정은 도는데 `area_02`의 **방이 하나도 없어** 해금돼도 목록에서 빠졌다 | 2026-08-26 **T26에서 페리온 8방**이 실제로 섰고, 이어 **엘리니아 `area_03`**(T36, 5방) · **커닝시티 `area_04`**(T39~T40, 8방)까지 들어갔다. ⚠ 다만 **소급 지급이 깨져 있어 실제로는 안 열리고 있었다** — 2026-08-29 **T40-1**에서 고쳤다(아래) |
...
| 3 | **차수 전직의 체감** — 잠겨 있던 히어로 스킬이 풀릴 때 | Lv20 → 슬래시 블러스트·아이언 바디 / Lv30 → 브랜디쉬 |
...
⚠ **오라 블레이드(3차)·레이징 블로우(4차)는 이번 회차에 못 낀다.** 3차가 Lv60인데
콘텐츠가 Lv35에서 끝난다. 히어로를 잡으면 5종이 다 들어오지만 2종만 쓸 수 있다 —
**의도된 것이 맞는지도 이번에 봐 주실 것.**
...
1. **팔라딘 · 다크나이트 NPC** — 히어로(T16-3)와 같은 방식. 지역이 늘 때 얹는 설계다.
   ⚠ 붙이면 모험가 쪽 스킬이 15종이 되어 **스킬 창이 잘린다**(`EquipPanel.Refresh`가
   넘칠 때 경고를 남긴다). 창 구조를 먼저 손봐야 한다.
2. **회복 스킬** — `effect_type=heal`은 배선만 돼 있고 데이터가 없다. **모험가 NPC 몫**이라
   위 1번이 생기면 CSV 한 줄로 붙는다.
3. **3차 콘텐츠(Lv60)** — 지금 콘텐츠가 Lv35에서 끝나 히어로 오라 블레이드(3차)·
   레이징 블로우(4차)는 **얻어도 못 낀다**. 갈아서 올리면 되지만 설계된 경로가 아니다.
...
- 기준: `main` (PR #137 머지) 위의 `feat/t33-5-invisible-walls`
- 머지된 PR 137개
