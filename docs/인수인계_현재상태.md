...
- 갱신: 2026-08-30 (T50 area_05 노틸러스)
- 대상: 이 프로젝트를 **처음 받는 사람 또는 AI**

> **이 문서의 목적 하나.** 기획서는 "이렇게 만들 것"을 적은 문서고, 코드는 "이렇게 만들어진 것"이다.
> 둘은 지금 **7군데에서 다르다** — 문서·지시에 있으나 없는 것 6건(C, D, F, J, **L**, **M**) + 문서 근거 없이 있는 것 1건(H).
...
| **L** | `MonsterTable`의 **커닝시티 5종 `drop_skill_id`** | 다섯 칸이 전부 비어 있다 → **커닝시티는 포획이 안 되는 지역**이다. 빈 값 자체는 안전하다(`TryCapture`가 즉시 빠져나가고 진척도 HUD도 스킬 줄을 건너뛴다) | T32(페리온)·T37(엘리니아)과 같은 3단계가 남았다: 데이터 → 아이콘 → 이펙트·소리. 대표 지시대로 **액티브 3 + 패시브 2**로 나눈다 |
| **M** | `MonsterTable`의 **노틸러스 5종 `drop_skill_id`** (T50) | L과 같다 — 다섯 칸이 비어 **노틸러스도 포획이 안 되는 지역**이다 | ⚠⚠ **이게 게이트 규격까지 밀어낸다.** 막다른 보상방은 *"앞 지역 보스 스킬(`key`)로 잠근다"*가 규격인데(커닝시티 T40-2 · 엘리니아 T43-1), **커닝시티 스킬이 없어 T50에서 `key` 대신 `stat 245`를 썼다.** 같은 이유로 area_06도 `key`를 못 건다 — **포획 스킬이 밀리면 지역 규격이 한 칸씩 계속 어긋난다** |
