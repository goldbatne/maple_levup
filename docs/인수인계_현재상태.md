...
- 갱신: 2026-08-30 (T52 몬스터 지역·보스·스킬·이펙트 정합성 감사)
...
> **현재 이어갈 지점(T52 이후):** Maker Refresh → 새 `Mushmom.model` 등록 확인 → `r_05` 머쉬맘,
> `r_045` 셰이드, `r_29`의 `s_mon_mushmom` 문, `r_05a`의 `s_mon_shade` 문을 실제로 검증한다.
> 그 다음 구현 지역은 **area_06 지하 하수도(Lv51~60)**다. area_06~20 보스안은
