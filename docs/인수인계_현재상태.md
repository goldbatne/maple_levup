...
- **몬스터 공격 주기 0.5초 · 추격 범위 6.0** — 13개 모델이 **전부 같은 값**이라 합칠 수는 있다.
  ⚠ **다만 합치면 종별로 다르게 둘 여지가 사라진다.** T28에서 "난이도는 곡선이 아니라
  방이 만든다"고 정했는데, 종별 공속·추격 범위는 그 방을 짜는 지렛대다.
  옮기려면 "CSV가 기본값, 모델이 덮어쓴다" 꼴이어야 하고 그건 코드가 는다. **대표 판단 대기.**
...
⚠ **2026-08-28(T33-1) 재확인 — 위의 "어떤 모델·맵에도 부착돼 있지 않고"는 우리 게임 기준으로만 맞다.**
전수 재조사 결과 **템플릿 잔재 5개가 이 11개를 참조한다**:
`Global/ChaseMonster.model` · `Global/MoveMonster.model` ·
`map/TileMapTemplate.map` · `map/RectTileMapTemplate.map` · `map/SideViewRectTileMapTemplate.map`.
우리 게임 모델 13개와 게임 맵(map01~map17 · maptown · mapjob)은 **하나도 안 쓴다.**

**그래서 지금 지우면 저 5개에 끊어진 참조가 남는다.** 지우려면 템플릿 잔재를 같이 정리해야 하는데,
`Global/`은 **삭제 금지**(AGENTS.md — 편집만 가능)이고 템플릿 맵 3개는 **대표 확인 없이 못 지운다.**
→ **묶어서 결정할 일이다.** 대표 판단 대기.
