...
- 갱신: 2026-08-28 (T33-3 몬스터 공속·추격 범위 CSV)
...
### 4.4 죽은 모델 (T33-3에서 발견)

`RootDesk/MyDesk/Models/Monsters/Slime.model` — `MonsterId = m_slime`인데 그 id가
`MonsterTable`에 없고 **어떤 맵도 안 쓴다.** T33-1에서 지운 `Skeleton.model`과 같은 경우다.
(헤네시스 재테마 2026-08-16 때 `m_slime`이 표에서 빠졌다.)
**T33-3의 범위 밖이라 안 지웠다** — 지울지 대표 판단 대기.

곁들여 `Combat/RoomMonster.mlua`의 `property string MonsterId = "m_slime"` 기본값도
없는 id를 가리킨다. 배치된 몬스터는 전부 값을 덮으므로 해는 없지만 낡은 값이다.
### 4.4 코드 내 TODO — 1건
