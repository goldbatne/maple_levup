...
수직 슬라이스(방 5개 / 몬스터 4종 / 직업 2종 / 환생 1회)가 **구현 완료** 상태다.
남은 것은 대표의 직접 플레이 판정과 수치 조율뿐이다 — [현재 상태](docs/인수인계_현재상태.md) 참고.
...
| # | 문서 | 무엇을 얻는가 |
|---|---|---|
| 1 | **[CLAUDE.md](CLAUDE.md)** | 프로젝트 규칙 · 현재 진행 상태 · 설계 변경 이력. **여기서 시작한다** |
| 2 | **[docs/인수인계_현재상태.md](docs/인수인계_현재상태.md)** | 무엇이 실제로 돌아가고 무엇이 문서에만 있는지. **기획서를 읽기 전에 이걸 먼저 읽어야 한다** |
| 3 | **[docs/게임기획서_v0.3.md](docs/게임기획서_v0.3.md)** | 시스템 설계 전체 (게이트 3종 · 슬롯 경제 · 환생 · 스탯). ⚠ 여기 적힌 것 중 **7개는 아직 구현되지 않았다** — 2번 문서 3절 참고 |
| 4 | **[docs/밸런스_확정수치.md](docs/밸런스_확정수치.md)** | 레벨 곡선 · 몬스터 계단 · 방별 위험도. **수치는 여기가 정본** |
| 5 | **[docs/작업지시서_v2.0.md](docs/작업지시서_v2.0.md)** | T0~T10 태스크별 구현 기록과 실측. "왜 이렇게 됐나"의 근거 |
| 6 | **[AGENTS.md](AGENTS.md)** | MSW 플랫폼 규칙 (AI 작업자 전용). CLAUDE.md가 자동 import한다. **`.model`/`.ui`/`.map` 편집 방법이 여기 있다** |

> 1~4번은 40분이면 읽는다. 5번은 필요할 때 해당 태스크 절만 찾아 읽으면 된다.

`docs/프로젝트_재정리_개발운영_가이드_v1.0.md`는 **히스토리 문서**다. 현행 기준이 아니다.
`docs/밸런스시트_v0.1.xlsx` / `v0.2.xlsx`는 **작업용 스프레드시트**다 — 읽기용 정본은 위 4번.
...
RootDesk/MyDesk/
  GameData/      GameData.mlua ...... 모든 정적 데이터 조회의 유일한 창구 (@Logic)
                 *.csv + *.userdataset  6종 — 밸런스/스킬/몬스터/방/직업/랜드마크
